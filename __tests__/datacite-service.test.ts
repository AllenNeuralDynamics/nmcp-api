...
describe("payloads", () => {
    test("createDoi sends the event it is given", async () => {
        const fetchMock = respondWith(201, successBody);
        vi.stubGlobal("fetch", fetchMock);
    ...
    test("updateDoi PUTs only the related identifiers", async () => {
    ...
    // Nothing is created as a draft, and DataCite does not permit deleting a registered DOI - so there is no state the
    // client could withdraw even if it wanted to.
