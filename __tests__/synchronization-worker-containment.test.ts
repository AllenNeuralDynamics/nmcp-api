...
// outcomes and the assertion is on its total call count.  Every item also carries the claim and release the worker
// now calls around the phase.
const makeBatch = (count: number, fn: any) =>
    Array.from({length: count}, (_unused, idx) => ({
        id: `item-${idx}`,
        assess: fn,
        calculateStructureAssignments: fn,
        assignDois: fn,
        updateSearchIndex: fn,
        claim: vi.fn().mockResolvedValue(true),
        release: vi.fn().mockResolvedValue(undefined)
    }));

const failsOnce = () => vi.fn().mockRejectedValueOnce(new Error("boom")).mockResolvedValue(PhaseOutcome.Handled);
const alwaysFails = () => vi.fn().mockRejectedValue(new Error("boom"));
const alwaysSucceeds = () => vi.fn().mockResolvedValue(PhaseOutcome.Handled);
...
function stubPhases(qc: any[], structure: any[], doi: any[], indexable: any[]) {
    return {
        ...
        // performSynchronization sweeps before its first phase, so these have to be stubbed or the pass reaches the
        // real models.
        releasePhaseClaims: vi.spyOn(AtlasReconstruction, "releasePhaseClaims").mockResolvedValue(0),
        releaseQualityControlClaims: vi.spyOn(QualityControl, "releasePhaseClaims").mockResolvedValue(0)
    };
}
